...
async function retryHubCallWithExponentialBackoff<T>(
  ...
  try {
    ...
    log.warn(
      `Error in backfill, attempt ${currentAttempt}/${maxAttempts}, retrying in ${delayMs}ms`
    );
...
export async function* getOnChainEventsByFidInBatchesOf(
  ...
  const hasSubTypeFilter =
    signerEventTypes?.length || idRegisterEventTypes?.length;
  ...
  for (const eventType of eventTypes) {
    ...
    for (;;) {
      ...
      result = await hub.getOnChainEvents({
        pageSize,
        pageToken,
        fid,
        eventType,
  ...
  return events.filter((event) => {
    switch (event.type) {
      case OnChainEventType.EVENT_TYPE_SIGNER:
        ...
        return (
          signerEventTypes === undefined ||
          signerEventTypes.includes(event.signerEventBody.eventType)
        );
      ...
      case OnChainEventType.EVENT_TYPE_ID_REGISTER:
        ...
        return (
          idRegisterEventTypes === undefined ||
          idRegisterEventTypes.includes(event.idRegisterEventBody.eventType)
        );
...
export async function getUserNameProofsByFid(hub: HubRpcClient, fid: number) {
  const result = await retryHubCallWithExponentialBackoff(() =>
    hub.getUserNameProofsByFid({ fid })
  );
  if (result.isErr()) {
...
export async function* getCastsByFidInBatchesOf(
  hub: HubRpcClient,
  fid: number,
  pageSize: number
) {
  let result = await retryHubCallWithExponentialBackoff(() =>
    hub.getCastsByFid({ pageSize, fid })
  );
  for (;;) {
    if (result.isErr()) {
      throw new Error("Unable to backfill", { cause: result.error });
    ...
    result = await retryHubCallWithExponentialBackoff(() =>
      hub.getCastsByFid({ pageSize, pageToken, fid })
    );
  }
}

export async function* getReactionsByFidInBatchesOf(
  hub: HubRpcClient,
  fid: number,
  pageSize: number
) {
  let result = await retryHubCallWithExponentialBackoff(() =>
    hub.getReactionsByFid({ pageSize, fid })
  );
  for (;;) {
    if (result.isErr()) {
      throw new Error(`Unable to fetch Reaction messages for FID ${fid}`, {
        cause: result.error,
      });
    }
    ...
    if (!pageToken?.length) break;
    result = await retryHubCallWithExponentialBackoff(() =>
      hub.getReactionsByFid({ pageSize, pageToken, fid })
    );
  }
}

export async function* getLinksByFidInBatchesOf(
  hub: HubRpcClient,
  fid: number,
  pageSize: number
) {
  let result = await retryHubCallWithExponentialBackoff(() =>
    hub.getLinksByFid({ pageSize, fid })
  );
  for (;;) {
    if (result.isErr()) {
      throw new Error(`Unable to fetch Link messages for FID ${fid}`, {
        cause: result.error,
      });
    }
...
export async function* getVerificationsByFidInBatchesOf(
  hub: HubRpcClient,
  fid: number,
  pageSize: number
) {
  let result = await retryHubCallWithExponentialBackoff(() =>
    hub.getVerificationsByFid({ pageSize, fid })
  );
  for (;;) {
    if (result.isErr()) {
      throw new Error(`Unable to fetch Verification messages for FID ${fid}`, {
        cause: result.error,
      });
    }
...
export async function* getUserDataByFidInBatchesOf(
  hub: HubRpcClient,
  fid: number,
  pageSize: number
) {
  let result = await retryHubCallWithExponentialBackoff(() =>
    hub.getUserDataByFid({ pageSize, fid })
  );
  for (;;) {
    if (result.isErr()) {
      throw new Error(`Unable to fetch UserData messages for FID ${fid}`, {
        cause: result.error,
      });
    }
    ...
    if (!pageToken?.length) break;
    result = await retryHubCallWithExponentialBackoff(() =>
      hub.getUserDataByFid({ pageSize, pageToken, fid })
    );
  }
}
